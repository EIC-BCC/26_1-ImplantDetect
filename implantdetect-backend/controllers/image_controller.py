...
async def submit_image(image: UploadFile, db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
...
async def get_user_images(db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
    user_id = int(user["sub"])
    image_service = ImageService(db)
...
async def get_image(image_id: int, db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
    image_service = ImageService(db)
