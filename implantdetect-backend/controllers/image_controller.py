...
async def submit_image(image: UploadFile, user_id: int, db=Depends(get_async_db)):
...
@router.get("/user/{user_id}/images", response_model=Result)
async def get_user_images(user_id: int, db=Depends(get_async_db), user=Depends(get_current_user)):
    image_service = ImageService(db)
    images = await image_service.get_all_images_from_user(user_id)
    return Result.ok(data={"images": [img.model_dump() for img in images]})

@router.get("/image/{image_id}", response_model=Result)
