...
async def get_process(process_id: int, db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
...
async def get_user_processes(db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
...
async def get_process_results(process_id: int, db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
    process_service = ProcessService(db)
