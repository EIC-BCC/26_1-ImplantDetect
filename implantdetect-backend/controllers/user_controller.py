...
async def update_user(user: UserUpdateRequest, db: AsyncSession = Depends(get_async_db)):
    user_service = UserService(db)
...
async def get_user(user_id: int, db: AsyncSession = Depends(get_async_db), user: JWTPayload = Depends(get_current_user)):
    user_service = UserService(db)
