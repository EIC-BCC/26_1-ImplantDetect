...
class ImageService:
    ...
    async def _validate_image(self, image: UploadFile) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Imagem {image.filename} validada com sucesso: "
    ...
    async def handle_image_upload(
        ...
        try:
            ...
            existing_image = await self.image_dao.get_image_by_hash(
                file_hash, file_extension
            )

            if existing_image:
                logger.info(
                    f"Imagem já existe no banco de dados com ID {existing_image.id}."
                )
