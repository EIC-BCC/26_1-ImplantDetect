...
import { Users, Activity, CheckCircle, AlertTriangle, Clock, Loader2, ArrowRight } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const [u, p] = await Promise.all([adminService.getUsers(), adminService.getProcesses()]);
        setUsers(u);
        setProcesses(p);
      } catch (err) {
        setError(err?.detail || err?.message || 'Erro ao carregar dados do painel.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const completed = processes.filter((p) => p.status_name === 'Concluído').length;
  const failed = processes.filter((p) => p.status_name === 'Falhou').length;
  const successRate = totalProcesses > 0 ? ((completed / totalProcesses) * 100).toFixed(1) : '—';
        ...
        <div className="lg:col-span-2">
          <Card>
            <Card.Header>
              ...
              <Card.Description>Últimos 8 processos no sistema</Card.Description>
            </Card.Header>
            ...
            {loading ? (
              ...
              <div className="divide-y divide-gray-50">
                {recentProcesses.map((p) => (
                  <div key={p.process_id} className="flex items-center justify-between py-3 gap-4">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900">
                        Processo #{p.process_id}
                        <span className="text-gray-400 font-normal ml-2 text-xs">usuário #{p.user_id}</span>
                      </p>
          ...
          <Card className="h-full">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {[
                  { role: 'admin', label: 'Administradores', color: 'bg-red-100 text-red-700' },
                  { role: 'specialist', label: 'Especialistas', color: 'bg-accent-100 text-accent-700' },
                  { role: 'user', label: 'Usuários', color: 'bg-primary-100 text-primary-700' },
