      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Visualize e gerencie as contas do sistema
          </p>
        </div>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="relative inline-block">
                        <button
                          onClick={() =>
                            setActionMenu(
                              actionMenu === user.user_id ? null : user.user_id,
                            )
                          }
                        ...
                        {actionMenu === user.user_id && (
                          <div className="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border py-1 z-10 animate-fade-in">
                            {ROLES.filter((r) => r !== user.role).map(
                              (role) => (
                                <button
                                  key={role}
                                  onClick={() =>
                                    handleSetRole(user.user_id, role)
                                  }
                                  className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center gap-2"
                                >
                                  <Shield className="h-4 w-4" /> Tornar{" "}
                                  {roleLabels[role]}
                                </button>
                              ),
                            )}
                            <button
                              onClick={() =>
                                handleToggleActive(user.user_id, user.active)
                              }
